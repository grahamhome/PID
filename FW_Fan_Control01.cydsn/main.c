/*********************************************************************************
* Project Name:      PID Control Algorithm - Performance Engineering
* Version:           1.0
*
* Device Used:       PSoC 5 CY8C5588AXI-060ES1
* Software Used:     PSoC Creator v3.
* Authors:           Graham Home
                     Alberto Santos-Castro
*********************************************************************************
* Theory of Operation:
*
*  The height of a ping pong ball is controlled by a Fan using a PID algorithm
*  The height measurement is obtained by an ultrasonic sensor
*  THe control signal of the PID algorithm is used as the input (Duty Cycle) to the
*  Fan Controller
*  The firmware synchornizes to hardware using the end-of-cycle (eoc) pulse
*  1 Fans is supported (individual PWMs - no banks)
...
#define LCD_SETPOINT    (LCD_Position(0u,  0u))
#define LCD_ECHO        (LCD_Position(0u,  6u))
#define LCD_DELTA       (LCD_Position(0u, 12u))
#define LCD_CTRL_SIG    (LCD_Position(1u,  0u))
#define LCD_DUTY        (LCD_Position(1u,  6u))
